...
const TabListAbout: React.FC<TabsProps> = ({ aboutData }) => {
  const [childrenData, setChildrenData] = useState<Tab | undefined>(aboutData.find(c => c.id === "1"));
  const [images, setImages] = useState<string[]>([]);
  const hendelOpenData = (id: string) => {
    setChildrenData(undefined); // Change this to undefined
    if (id) {
      const findData = aboutData.find(c => c.id === id);
      setChildrenData(findData);
    }
  }
      ...
      <div className="mt-16">
        {
          childrenData?.id === '1' ?
            ...
            : <>
              ...
              <img src={images.length > 0 ? images[0] : childrenData?.client?.location} alt="" className="h-[400px] w-full"/>
